...
import { ReplicateClient, getVideoAssetsDir } from './replicate-client'
import { buildShotPrompt, getShotTemplate } from './video-asset-prompts'
import { buildLogoAnimationPrompt, getLogoAnimationTemplate } from './logo-animation-prompts'
...
ipcMain.handle('ai:upscale-runway', async (_e: any, args: { input: string; output: string }) => {
  const { input, output } = args
  ...
  const client = new ReplicateClient({ apiToken: replicateApiKey })
  ...
  try {
    const upscaledPath = await client.upscaleVideo(
      input,
      outputDir,
      (status) => {
...
ipcMain.handle('settings:setApiKey', async (_e: any, payload: { provider: string; key: string }) => {
  const config = await loadConfig()
  if (!config.api_keys) config.api_keys = {}
...
ipcMain.handle('settings:removeApiKey', async (_e: any, provider: string) => {
  const config = await loadConfig()
...
async function processVideoAssetJob(
  jobId: string,
  type: VideoAssetJobType,
  shotPresetIds: string[],
  imagePaths: string[],
...
async function processAIVideoPack(
  jobId: string,
  shotPresetIds: string[],
  imagePaths: string[],
  model: 'runway' | 'veo' = 'runway'
  ...
  const client = new ReplicateClient({ apiToken: replicateApiKey })
  const outputDir = path.join(getVideoAssetsDir(), jobId)
  
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true })
  }

  const resultAssets: VideoAssetShotResult[] = []
  ...
  const shotPromises = shotPresetIds.map(async (shotId) => {
    ...
    const prompt = buildShotPrompt(shotId, 'the product on display')
    ...
    try {
      ...
      if (model === 'veo') {
        ...
        videoPath = await client.generateVideoVeo(
          primaryImage,
          prompt,
          outputDir,
          (status) => {
        ...
        videoPath = await client.generateVideo(
          primaryImage,
          prompt,
          outputDir,
          (status) => {
...
async function processLogoAnimations(
  jobId: string,
  animationIds: string[],
  logoPaths: string[],
  model: 'runway' | 'veo' = 'runway'
  ...
  const animationPromises = animationIds.map(async (animationId) => {
    ...
    const prompt = buildLogoAnimationPrompt(animationId, 'brand logo')
...
async function updateVideoAssetJobStatus(
  jobId: string,
  status: 'pending' | 'running' | 'completed' | 'failed',
  error: string | null
): Promise<void> {
