...
let currentEnhanceCommand = null;
...
ipcMain.handle('screen:save-recording', async (_e, args) => {
...
ipcMain.handle('recording:cleanupTempDir', async (_e, tempDir) => {
    try {
...
ipcMain.handle('ai:enhance', async (_e, args) => {
    const { input, output } = args;
...
const getVideoAssetsConfig = async () => {
    const config = await loadConfig();
    if (!config.video_asset_jobs) {
        config.video_asset_jobs = [];
    }
    return { jobs: config.video_asset_jobs };
};
const saveVideoAssetsConfig = async (jobs) => {
    const config = await loadConfig();
...
ipcMain.handle('room:identifyRooms', async (_e, imagePathOrBase64, detections, isBase64) => {
    try {
        ...
        const roomsInfo = detections.map((det, idx) => `Room ${idx + 1} (ID: ${det.id}): Bounding box at [${det.bounding_box.join(', ')}]`).join('\n');
...
ipcMain.handle('damage:estimateCost', async (_e, imagePathOrBase64, detections, isBase64) => {
    try {
        // Check rate limit first
        const rateLimit = checkRateLimit();
        if (!rateLimit.allowed) {
