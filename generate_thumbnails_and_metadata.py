"""
OVERNIGHT BATCH JOB: Generate Thumbnails + Extract Geometry Metadata
Run this script INSIDE Omniverse Kit's Script Editor.

Extracts:
- Thumbnails (512x512 PNG)
- Bounding box dimensions (width, height, depth in world units)
- Default scale and position
- Asset category (based on prim types)
- Material count

ESTIMATED TIME: 4-8 hours for 3,092 assets
Progress saved every 50 assets (resumable if interrupted)

BEFORE RUNNING:
1. Close any open scenes (File → New)
...
from pxr import Usd, UsdGeom, Gf, UsdShade, UsdLux
...
PROGRESS_FILE = "Z:/omniverse_assets/asset_catalog/processing_progress.json"

print("="*80)
print("THUMBNAIL + METADATA GENERATOR - Overnight Batch Processing")
print("="*80)
...
def extract_metadata(stage, usd_file):
    """Extract geometry and metadata from a USD stage."""
    metadata = {
        'bbox_width': None,
        'bbox_height': None,
        'bbox_depth': None,
    ...
    try:
        # Get default prim or root
        prim = stage.GetDefaultPrim()
        if not prim:
            prim = stage.GetPseudoRoot()
        
        # Count prims and categorize
        mesh_count = 0
        material_count = 0
        has_camera = False
        has_light = False
        ...
        for p in Usd.PrimRange(prim):
            metadata['prim_count'] += 1
            
            if p.IsA(UsdGeom.Mesh):
                mesh_count += 1
            elif p.IsA(UsdShade.Material):
                material_count += 1
        ...
        # Determine category
        if has_camera and has_light:
            metadata['category'] = 'scene'
        elif mesh_count > 50:
        ...
        if bounds:
            ...
            if not bbox_range.IsEmpty():
                ...
                metadata['bbox_width'] = float(width)
                metadata['bbox_height'] = float(height)
                metadata['bbox_depth'] = float(depth)
                
                center = bbox_range.GetMidpoint()
                metadata['bbox_center'] = [float(center[0]), float(center[1]), float(center[2])]
        
        # Get default transform if available
        if prim and UsdGeom.Xformable(prim):
            ...
            for op in xform_ops:
                if op.GetOpType() == UsdGeom.XformOp.TypeScale:
                    scale = op.Get()
                    if scale:
                        metadata['default_scale'] = [float(scale[0]), float(scale[1]), float(scale[2])]
                elif op.GetOpType() == UsdGeom.XformOp.TypeTranslate:
                    pos = op.Get()
                    if pos:
                        metadata['default_position'] = [float(pos[0]), float(pos[1]), float(pos[2])]
...
# ============================================================================
print("\n" + "="*80)
print("STARTING THUMBNAIL + METADATA GENERATION")
print("="*80)
...
for idx, asset in enumerate(catalog['assets'], 1):
    ...
    try:
        ...
        # Extract metadata
        metadata = extract_metadata(stage, usd_file)
        
        # Update asset with metadata
        asset.update(metadata)
        
        # Frame the asset in viewport
        # Use Kit command to frame selection
        omni.kit.commands.execute('FramePrimsCommand',
            prim_to_move=viewport_api.get_active_viewport_camera_path(),
            prims_to_frame=[stage.GetDefaultPrim().GetPath() if stage.GetDefaultPrim() else stage.GetPseudoRoot().GetPath()])
        time.sleep(0.3)
        ...
        if Path(thumbnail_path).exists():
            ...
            success_count += 1
            if idx % 10 != 0:
                print(f"[{idx}] ✓ {asset['name']} (bbox: {metadata['bbox_width']:.1f}x{metadata['bbox_height']:.1f}x{metadata['bbox_depth']:.1f})")
        else:
...
print("\n" + "="*80)
print("🎉 THUMBNAIL + METADATA GENERATION COMPLETE!")
print("="*80)
...
print("\n2. Sync updated catalog to S3:")
print("   aws s3 cp Z:/omniverse_assets/asset_catalog/master_index.json \\")
print("     s3://clappper-assets/omniverse_assets/asset_catalog/master_index.json")
print("\n3. Review errors (if any) in processing_progress.json")
print("\n4. Optional: Generate visual embeddings from thumbnails")
print("="*80)
print("\n🌙 Good night! See you in the morning with 3,092 thumbnails + metadata!")
print("="*80)
