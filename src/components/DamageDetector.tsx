...
export default function DamageDetector({ isOpen, onClose }: DamageDetectorProps) {
  ...
  const handleDownloadEstimate = async () => {
    if (!result?.cost_estimate) return
    ...
    try {
      ...
      // Format as text
      const content = `ROOF DAMAGE REPAIR COST ESTIMATE
Generated: ${timestamp}
Source: GPT-4 Vision AI Analysis

================================================

COST BREAKDOWN:

Labor:              $${estimate.labor_usd.toFixed(2)}
Materials:          $${estimate.materials_usd.toFixed(2)}
Disposal:           $${estimate.disposal_usd.toFixed(2)}
Contingency (10%):  $${estimate.contingency_usd.toFixed(2)}

------------------------------------------------
TOTAL ESTIMATE:     $${estimate.total_usd.toFixed(2)}
================================================
...
DISCLAIMER:
This estimate was generated by AI for demonstration purposes only.
Please consult with licensed roofing contractors for professional
assessments and accurate pricing for your specific project.
`
      ...
      const originalName = selectedImage?.split(/[/\\]/).pop()?.replace(/\.[^.]+$/, '') || 'roof_damage'
      const filename = `${originalName}_estimate.txt`
      ...
      setError(`Failed to download estimate: ${err instanceof Error ? err.message : 'Unknown error'}`)
    }
  }

  const handleDownloadPDF = async () => {
    if (!result?.annotated_image || !result?.cost_estimate) return
    ...
    try {
      ...
      pdf.text('ROOF DAMAGE REPAIR REPORT', margin, margin)
      
      // Add timestamp and source
      pdf.setFontSize(9)
      pdf.setFont('helvetica', 'normal')
      pdf.text(`Generated: ${timestamp}`, margin, margin + 7)
      pdf.text('Source: GPT-4 Vision AI Analysis', margin, margin + 12)
      
      // Add image
      const imageData = `data:image/png;base64,${result.annotated_image}`
      const imgWidth = pageWidth - (2 * margin)
      const imgHeight = 80 // Fixed height to ensure everything fits
      pdf.addImage(imageData, 'PNG', margin, margin + 18, imgWidth, imgHeight)
      
      let yPos = margin + 18 + imgHeight + 10
      ...
      pdf.text(`$${estimate.labor_usd.toFixed(2)}`, pageWidth - margin - 20, yPos, { align: 'right' })
      yPos += 5
      
      pdf.text(`Materials:`, margin, yPos)
      pdf.text(`$${estimate.materials_usd.toFixed(2)}`, pageWidth - margin - 20, yPos, { align: 'right' })
      yPos += 5
      
      pdf.text(`Disposal:`, margin, yPos)
      pdf.text(`$${estimate.disposal_usd.toFixed(2)}`, pageWidth - margin - 20, yPos, { align: 'right' })
      yPos += 5
      
      pdf.text(`Contingency:`, margin, yPos)
      pdf.text(`$${estimate.contingency_usd.toFixed(2)}`, pageWidth - margin - 20, yPos, { align: 'right' })
      yPos += 7
      
      // Total
      pdf.setFont('helvetica', 'bold')
      pdf.text(`TOTAL ESTIMATE:`, margin, yPos)
      pdf.text(`$${estimate.total_usd.toFixed(2)}`, pageWidth - margin - 20, yPos, { align: 'right' })
      yPos += 10
      
      // Assumptions (with text wrapping)
      pdf.setFontSize(10)
      pdf.setFont('helvetica', 'bold')
      ...
      pdf.setFont('helvetica', 'normal')
      pdf.setFontSize(8)
      const splitAssumptions = pdf.splitTextToSize(estimate.assumptions, pageWidth - (2 * margin))
      pdf.text(splitAssumptions, margin, yPos)
      yPos += (splitAssumptions.length * 3) + 5
      
      // Disclaimer
      pdf.setFont('helvetica', 'italic')
      pdf.setFontSize(7)
      const disclaimer = 'DISCLAIMER: This estimate was generated by AI for demonstration purposes only. Please consult with licensed roofing contractors for professional assessments and accurate pricing.'
      ...
      setError(`Failed to generate PDF: ${err instanceof Error ? err.message : 'Unknown error'}. Make sure jsPDF is installed.`)
    }
  }
                    ...
                    <div style={{
                      ...
                      <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #dee2e6' }}>
                        <div style={{ fontSize: 10, color: '#6c757d', marginBottom: 6 }}>
                          Enter your zip code to search for contractors in your area
                        </div>
